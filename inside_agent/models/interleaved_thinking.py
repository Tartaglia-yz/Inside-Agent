...
class InterleavedThinkingModel(BaseModel):
    ...
    def __init__(self, base_model: BaseModel, tools: List[Any] = None, os_info: Dict[str, str] = None):
        ...
        self.tools = tools or []
        self.os_info = os_info or (getattr(base_model, 'os_info', None) if hasattr(base_model, 'os_info') else None)
...
**当前系统环境：**
- 操作系统：{os_info.get('os_type', 'unknown').upper()}
- 默认Shell：{os_info.get('shell', 'bash')}
- 列出目录命令：{os_info.get('list_dir', 'ls')}
- 当前目录命令：{os_info.get('current_dir', 'pwd')}

**重要：请严格根据当前操作系统生成对应的命令！**
"""
...
"""
    ...
    def generate(self, context: List[Dict[str, Any]]) -> Dict[str, Any]:
        """生成模型响应，支持ReAct循环（启动时注入工具信息）"""
        react_context = context.copy()
        
        # 启动时立即注入工具信息和系统信息
        self.logger.info("启动时注入工具信息和系统信息")
        ...
        for tool_call in tool_calls:
            ...
            if tool:
                try:
                    ...
                    # 无论执行成功还是失败，都添加系统信息和正确的命令格式
                    os_info = self.os_info or {}
                    os_type = os_info.get('os_type', 'unknown')
                    
                    # 添加系统信息和命令示例
                    system_hint = "\n\n[系统信息] 当前系统：" + os_type.upper() + "\n"
                    
                    if os_type == 'windows':
                        system_hint += "[推荐命令]\n"
                        system_hint += "- 查看系统时间：time\n"
                        system_hint += "- 列出目录：dir\n"
                        system_hint += "- 查看文件：type filename.txt\n"
                        system_hint += "- 查看当前目录：cd\n"
                    else:
                        system_hint += "[推荐命令]\n"
                        system_hint += "- 查看系统时间：date\n"
                        system_hint += "- 列出目录：ls -la\n"
                        system_hint += "- 查看文件：cat filename.txt\n"
                        system_hint += "- 查看当前目录：pwd\n"
                    ...
                    if os_type == 'windows':
                        error_message += "[推荐命令]\n"
                        error_message += "- 查看系统时间：time\n"
                        error_message += "- 列出目录：dir\n"
                        error_message += "- 查看文件：type filename.txt\n"
                        error_message += "- 查看当前目录：cd\n"
                    else:
                        error_message += "[推荐命令]\n"
                        error_message += "- 查看系统时间：date\n"
                        error_message += "- 列出目录：ls -la\n"
                        error_message += "- 查看文件：cat filename.txt\n"
                        error_message += "- 查看当前目录：pwd\n"
    ...
    def generate_stream(self, context: List[Dict[str, Any]]) -> str:
        """流式生成模型响应，支持ReAct工具调用（启动时注入工具信息）"""
        react_context = context.copy()
